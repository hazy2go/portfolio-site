...
// ===== Hand Catch Animation =====
function initHandCatchAnimation() {
    const section = document.querySelector('.hand-catch-animation');
    if (!section) return;

    const words = section.querySelectorAll('.text-word');
    const hand = section.querySelector('.hand-svg');
    const handParts = section.querySelectorAll('.hand-palm, .hand-finger, .hand-thumb');
    const glow = section.querySelector('.hand-glow');
        ...
        scrollTrigger: {
            ...
            onUpdate: (self) => {
                ...
                if (self.progress > 0.6) {
                    section.classList.add('text-assembled');
                }
            }
        }
    });

    // Phase 1: Hand appears (0% - 30% scroll)
    tl.fromTo(handParts, 
        { 
            opacity: 0, 
            y: 50,
    ...
    // Phase 2: Words appear scattered (20% - 40% scroll)
    tl.fromTo(words,
        {
            opacity: 0,
            y: -100,
    ...
    // Phase 3: Words fall into hand (40% - 70% scroll)
    tl.to(words, {
        y: 0,
        rotation: 0,
        duration: 0.6,
        ...
        ease: "power2.inOut",
        onComplete: () => {
            // Add bounce effect when words "land"
            gsap.to(words, {
                scale: 1.1,
                duration: 0.1,
                yoyo: true,
    ...
    // Phase 4: Hand glow and particles (60% - 100% scroll)
    tl.to(glow,
        {
            opacity: 0.4,
            scale: 1.2,
    ...
    // Add hover effect to the entire section
...
// Alternative: Intersection Observer for browsers without GSAP
function initHandCatchAnimationFallback() {
    const section = document.querySelector('.hand-catch-animation');
    ...
    initHandCatchAnimation();
}

// Initialize when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
    // Small delay to ensure GSAP is loaded
    setTimeout(initHandCatchAnimationFallback, 100);
});

// Refresh ScrollTrigger when page loads (for cached pages)
